# Generative Cover Pilot Checklist

## Source

- [ ] Interior PDF path is recorded.
- [ ] Page count was checked with `pdfinfo` or equivalent.
- [ ] Final interior status is known.
- [ ] Spine is marked `estimativa_nao_final` unless confirmed by the platform.

## Base Art

- [ ] Base art is preserved as its own file.
- [ ] Base art has no title, letters, logo, signature, watermark, or accidental readable marks.
- [ ] Base art communicates the genre, topic, and reading promise.
- [ ] Prompt is saved in the pilot folder.
- [ ] Origin is recorded: AI generation, licensed image, manual collage, or other.

## Composition

- [ ] Title, subtitle, author, spine, and back cover are composed locally.
- [ ] Text is spelled correctly.
- [ ] Accent marks and author name are correct.
- [ ] Back-cover copy is editorial/commercial, not technical process text.
- [ ] Barcode/ISBN area is reserved when required.

## Export

- [ ] Clean complete cover exists.
- [ ] Validation preview with guides exists separately.
- [ ] Clean complete cover has no guides, cut lines, or internal status labels.
- [ ] Image mode, dimensions, DPI, and format were checked.
- [ ] SHA-256 hashes were recorded.

## Visual Review

- [ ] Full-size review was completed.
- [ ] Thumbnail review was completed.
- [ ] Contrast and title hierarchy are acceptable.
- [ ] Spine text is readable if spine text is allowed.
- [ ] Bleed and safety margin look correct.
- [ ] The author approved the pilot before any batch generation.
