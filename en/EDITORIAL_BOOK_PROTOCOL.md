...
- Before generating, reinterpreting, or composing a cover, audit the visual asset library: `Capas/`, the book-specific directory, `imagens/capa`, `imagens/personagens`, scene folders, banners, and editable design files. Classify every source as canonical/approved, candidate, historical archive, or uncertain origin.
- In fiction, named characters, established settings, and narrative objects are canonical references until the author explicitly revises them. Do not generate a generic new appearance without reviewing existing references; create new art only to fill a gap, derive a coherent variation, or satisfy approved art direction.
- If a strong, coherent complete cover already exists, use it as the starting point for template adaptation rather than as an excuse to reimagine the book. Record why new art is necessary and which references were used.
- The back cover must continue the front-cover visual language: extend the same scene, panorama, environment, texture, or narrative motif, while retaining meaningful image under the reading area. A tonal synopsis panel is acceptable only when it does not turn the back cover into an opaque, empty, or disconnected black field.
- It is prohibited to simply repeat or mirror the same vertical front-cover artwork on the back cover. Continuity requires one of these sources: (a) one panoramic artwork designed for the entire exterior, from which back and front are cropped at different positions; or (b) two distinct artworks with separate functions but coherent setting, characters, objects, period, lighting, palette, and art direction.
- Review front, spine, and back cover as a single open composition. Separately approve scene continuity, synopsis contrast, character coherence, barcode reserve, and absence of visual repetition across titles.
...
- Before approving the back cover, verify that contrast treatment did not erase the artwork or break visual continuity with the front. For fiction, also check characters, costumes, period, and settings against the audited canonical asset library.
- Record which method was used (full exterior panorama or two coherent scenes), the source files for each panel, and the human review confirming there is no literal repetition of the front artwork.
